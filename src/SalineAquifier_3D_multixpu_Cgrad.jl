...
@parallel function compute_Dflux!(qDx, qDy, qDz, Pf, T, C, k_ηf, _dx, _dy, _dz, αρg, _1_θ_dτ_D)
    @inn_x(qDx) = @inn_x(qDx) - (@inn_x(qDx) + k_ηf * (@d_xa(Pf) * _dx)) * _1_θ_dτ_D
    @inn_y(qDy) = @inn_y(qDy) - (@inn_y(qDy) + k_ηf * (@d_ya(Pf) * _dy)) * _1_θ_dτ_D
    @inn_z(qDz) = @inn_z(qDz) - (@inn_z(qDz) + k_ηf * (@d_za(Pf) * _dz - αρg * @av_za(T)+ αρg * @av_za(C))) * _1_θ_dτ_D
    return nothing
end
    ...
    for it = 1:nt
        ...
        while max(err_D, err_T, err_C) >= ϵtol && iter <= maxiter
            ...
            @hide_communication b_width begin
                @parallel compute_Dflux!(qDx, qDy, qDz, Pf, T, C, k_ηf, _dx, _dy, _dz, αρg, _1_θ_dτ_D)
        ...
        if do_viz &&  (it % nvis == 0) || it == 1
            ...
            if me == 0
                file = matopen("$(@__DIR__)/viz3D_out/SalineAqui_$(it)_nodT.mat", "w"); write(file, "T", Array(T_v));write(file, "C", Array(C_v)); close(file)
            end
        end
    end
